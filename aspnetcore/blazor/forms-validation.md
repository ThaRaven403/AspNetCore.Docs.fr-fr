---
title: Les formulaires et la validation ASP.NET Core Blazor
author: guardrex
...
---
# <a name="aspnet-core-no-locblazor-forms-and-validation"></a>Les formulaires et la validation ASP.NET Core Blazor
...
Les formulaires et la validation sont pris en charge dans Blazor avec l’utilisation des [Annotations de données](xref:mvc/models/validation).
...
Un formulaire est défini à l’aide du composant <xref:Microsoft.AspNetCore.Components.Forms.EditForm>. Le formulaire suivant illustre des éléments, des composants et du code Razor typiques :
...
* Le formulaire valide l’entrée utilisateur dans le champ `name` à l’aide de la validation définie dans le type `ExampleModel`. Le modèle est créé dans le bloc du composant `@code` et conservé dans un champ privé ( `exampleModel` ). Le champ est assigné à l'attribut `Model` de l'élément `<EditForm>`.
* `@bind-Value` du composant <xref:Microsoft.AspNetCore.Components.Forms.InputText> lie :
  * Propriété de modèle ( `exampleModel.Name` ) à la propriété `Value` du composant <xref:Microsoft.AspNetCore.Components.Forms.InputText>. Pour plus d’informations sur la liaison de propriété, consultez <xref:blazor/components/data-binding#parent-to-child-binding-with-component-parameters>.
  * Délégué d’événement de modification à la propriété `ValueChanged` du composant <xref:Microsoft.AspNetCore.Components.Forms.InputText>.
* Le [composant validateur](#validator-components) <xref:Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator> attache la prise en charge de la validation à l’aide d’annotations de données.
* Le composant <xref:Microsoft.AspNetCore.Components.Forms.ValidationSummary> résume les messages de validation.
...
> [!NOTE]
> Les composants `InputRadio` et `InputRadioGroup` sont disponibles dans ASP.net Core 5,0 ou version ultérieure. Pour plus d’informations, sélectionnez une version 5,0 ou ultérieure de cet article.
...
Le type `Starship` suivant définit la logique de validation à l’aide d’un plus grand ensemble de propriétés et d’annotations de données que la précédente `ExampleModel` :
...
Le formulaire suivant valide l’entrée d’utilisateur à l’aide de la validation définie dans le modèle `Starship` :
...
* La méthode `HandleSubmit` s’exécute lorsque le bouton **`Submit`** est sélectionné.
...
Dans l'exemple de composant `InputDate` suivant :

* Le nom complet ( `DisplayName` ) a la valeur `birthday` .
* Le composant est lié à la propriété `BirthDate` en tant que type `DateTime`.
...
* La banque de messages reçoit des erreurs quand le code du développeur dans le composant du formulaire appelle la méthode `DisplayErrors`. Les erreurs sont passées à la méthode `DisplayErrors` dans un [`Dictionary<string, List<string>>`](xref:System.Collections.Generic.Dictionary`2). Dans le dictionnaire, la clé est le nom du champ de formulaire qui contient une ou plusieurs erreurs. La valeur est la liste d’erreurs.
* Les messages sont effacés lorsque l’un des éléments suivants s’est produit :
  * La validation est demandée sur le <xref:Microsoft.AspNetCore.Components.Forms.EditContext> quand l'événement <xref:Microsoft.AspNetCore.Components.Forms.EditContext.OnValidationRequested> est déclenché. Toutes les erreurs sont effacées.
  * Un champ est modifié dans le formulaire lorsque l'événement <xref:Microsoft.AspNetCore.Components.Forms.EditContext.OnFieldChanged> est déclenché. Seules les erreurs du champ sont effacées.
...
* Le composant `CustomValidator` de la section [composants du validateur](#validator-components) de cet article est utilisé.
* La validation requiert une valeur pour la description du vaisseau ( `Description` ) si l’utilisateur sélectionne la classification de vaisseau (`Classification`) `Defense`.
...
* Traitez la validation côté client dans le formulaire avec le composant <xref:Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator>.
* Quand le formulaire passe la validation côté client (<xref:Microsoft.AspNetCore.Components.Forms.EditForm.OnValidSubmit> appelée), envoyez <xref:Microsoft.AspNetCore.Components.Forms.EditContext.Model?displayProperty=nameWithType> à une API de serveur principal pour le traitement de formulaire.
* Processus de validation du modèle sur le serveur.
* L’API serveur comprend la validation des annotations de données d’infrastructure intégrées et la logique de validation personnalisée fournie par le développeur. Si la validation réussit sur le serveur, traitez le formulaire et renvoyez un code d’état de réussite ( *200-OK* ). Si la validation échoue, retournez un code d’état d’échec ( *400-demande incorrecte* ) et les erreurs de validation de champ.
* Désactivez le formulaire en cas de réussite ou affichez les erreurs.

L’exemple suivant est basé sur :

* Solution hébergée Blazor créée par le [modèle de projet Blazor hébergé](xref:blazor/hosting-models#blazor-webassembly). L’exemple peut être utilisé avec l’une des solutions hébergées sécurisées Blazor décrites dans la [documentation Identity et la sécurité](xref:blazor/security/webassembly/index#implementation-guidance).
* Exemple *de formulaire de base de données Starfleet Starship* dans la section [composants de formulaires intégrés](#built-in-forms-components) précédente.
* Le composant <xref:Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator> du Framework Blazor.
* Le composant `CustomValidator` affiché dans la section [composants du validateur](#validator-components) .

Dans l’exemple suivant, l’API du serveur valide qu’une valeur est fournie pour la description du vaisseau ( `Description` ) si l’utilisateur sélectionne la classification de vaisseau (`Classification`) `Defense`.

Placez le modèle `Starship` dans le projet `Shared` de la solution afin que les applications client et serveur puissent utiliser le modèle. Étant donné que le modèle requiert des annotations de données, ajoutez une référence de package pour [`System.ComponentModel.Annotations`](https://www.nuget.org/packages/System.ComponentModel.Annotations) dans le fichier projet du projet `Shared` :
...
Si l’API serveur retourne la réponse JSON par défaut précédente, le client peut analyser la réponse pour obtenir les enfants du nœud `errors`. Toutefois, il est peu commode d’analyser le fichier. L’analyse du code JSON nécessite du code supplémentaire après l’appel à <xref:System.Net.Http.Json.HttpContentJsonExtensions.ReadFromJsonAsync%2A> afin de produire un [`Dictionary<string, List<string>>`](xref:System.Collections.Generic.Dictionary`2) des erreurs pour le traitement des erreurs de validation des formulaires. Dans l’idéal, l’API serveur doit renvoyer uniquement les erreurs de validation :
...
Dans le projet client, le formulaire *de base de données Starfleet Starship* est mis à jour pour afficher les erreurs de validation du serveur avec l’aide du composant `CustomValidator`. Lorsque l’API serveur retourne des messages de validation, ils sont ajoutés au `CustomValidator` du composant <xref:Microsoft.AspNetCore.Components.Forms.ValidationMessageStore>. Les erreurs sont disponibles dans le formulaire <xref:Microsoft.AspNetCore.Components.Forms.EditContext> pour être affichées par le <xref:Microsoft.AspNetCore.Components.Forms.ValidationSummary> du formulaire :
...
> [!NOTE]
> L’approche de validation côté serveur de cette section convient à l’un des exemples de solutions hébergées Blazor WebAssembly dans cet ensemble de documentation :
>
...
Utilisez le composant <xref:Microsoft.AspNetCore.Components.Forms.InputText> pour créer un composant personnalisé qui utilise l'événement `input` au lieu de l'événement `change`.

Dans l’exemple suivant, le composant `CustomInputText` hérite du composant de l’infrastructure `InputText` et définit la liaison d’événements ( <xref:Microsoft.AspNetCore.Components.EventCallbackFactoryBinderExtensions.CreateBinder%2A> ) sur l'événement `oninput`.
...
Le composant `CustomInputText` peut être utilisé partout où <xref:Microsoft.AspNetCore.Components.Forms.InputText> est utilisé :
...
Utilisez les composants `InputRadio` avec le composant `InputRadioGroup` pour créer un groupe de cases d’option. Dans l’exemple suivant, les propriétés sont ajoutées au modèle `Starship` décrit dans la section [composants de formulaires intégrés](#built-in-forms-components) :
...
Ajoutez les `enums` suivants à l’application. Créez un nouveau fichier pour contenir les `enums` ou ajoutez les `enums` au fichier `Starship.cs`. Rendez les `enums` accessibles au modèle `Starship` et à la *base de données Starfleet Starship* :
...
> [!NOTE]
> Si `Name` est omis, les composants `InputRadio` sont regroupés par leur ancêtre le plus récent.
...
L’exemple <xref:Microsoft.AspNetCore.Components.Forms.EditForm> suivant utilise le composant `InputRadio` précédent pour obtenir et valider une évaluation de l’utilisateur :
...
Il n’existe pas de méthode raisonnable pour représenter une valeur d’option d’élément `<select>` en tant que valeur d’objet C# `null` , car :

* Les attributs HTML ne peuvent pas avoir de valeurs `null`. L’équivalent le plus proche de `null` en HTML est l’absence de l'attribut `value` HTML de l'élément `<option>`.
* Quand vous sélectionnez un élément `<option>` sans attribut `value`, le navigateur traite la valeur comme le *contenu de texte* de cet élément `<option>`.

L'infrastructure Blazor ne tente pas de supprimer le comportement par défaut, car cela impliquerait :
...
Le composant <xref:Microsoft.AspNetCore.Components.Forms.ValidationSummary> résume tous les messages de validation, ce qui est similaire au [tag Helper de résumé de validation](xref:mvc/views/working-with-forms#the-validation-summary-tag-helper) :
...
Le composant <xref:Microsoft.AspNetCore.Components.Forms.ValidationMessage%601> affiche des messages de validation pour un champ spécifique, qui est similaire [au tag Helper de message de validation](xref:mvc/views/working-with-forms#the-validation-message-tag-helper). Spécifiez le champ pour la validation avec l'attribut `For` et une expression lambda qui nomme la propriété de modèle :

```razor
<ValidationMessage For="@(() => starship.MaximumAccommodation)" />
```

Les composants <xref:Microsoft.AspNetCore.Components.Forms.ValidationMessage%601> et <xref:Microsoft.AspNetCore.Components.Forms.ValidationSummary> prennent en charge des attributs arbitraires. Tout attribut qui ne correspond pas à un paramètre de composant est ajouté à l'élément `<div>` ou `<ul>` généré.
...
> [!NOTE]
> <xref:System.ComponentModel.DataAnnotations.ValidationContext.GetService%2A?displayProperty=nameWithType> est `null`. L’injection de services pour la validation dans la méthode `IsValid` n’est pas prise en charge.

::: moniker range=">= aspnetcore-5.0"

## <a name="custom-validation-class-attributes"></a>Attributs de classe de validation personnalisée

Les noms des classes de validation personnalisées sont utiles lors de l’intégration avec des frameworks CSS, tels que [bootstrap](https://getbootstrap.com/). Pour spécifier des noms de classe de validation personnalisés, créez une classe dérivée de `FieldCssClassProvider` et définissez la classe sur l'instance <xref:Microsoft.AspNetCore.Components.Forms.EditContext> :
...
### <a name="no-locblazor-data-annotations-validation-package"></a>Package de validation des annotations de données Blazor

Le [`Microsoft.AspNetCore.Components.DataAnnotations.Validation`](https://www.nuget.org/packages/Microsoft.AspNetCore.Components.DataAnnotations.Validation) est un package qui remplit les lacunes de l’expérience de validation à l’aide du composant <xref:Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator>. Le package est actuellement *expérimental*.
...
Le <xref:System.ComponentModel.DataAnnotations.CompareAttribute> ne fonctionne pas correctement avec le composant <xref:Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator>, car il n’associe pas le résultat de la validation à un membre spécifique. Cela peut entraîner un comportement incohérent entre la validation au niveau du champ et le moment où la totalité du modèle est validée sur une soumission. Le package [`Microsoft.AspNetCore.Components.DataAnnotations.Validation`](https://www.nuget.org/packages/Microsoft.AspNetCore.Components.DataAnnotations.Validation) *expérimental* introduit un attribut de validation supplémentaire, `ComparePropertyAttribute`, qui contourne ces limitations. Dans une application Blazor, `[CompareProperty]` est un remplacement direct de l'attribut [`[Compare]`](xref:System.ComponentModel.DataAnnotations.CompareAttribute).

### <a name="nested-models-collection-types-and-complex-types"></a>Modèles imbriqués, types de collection et types complexes

Blazor prend en charge la validation de l’entrée de formulaire à l’aide d’annotations de données avec le composant <xref:Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator> intégré. Toutefois, le <xref:Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator> valide uniquement les propriétés de niveau supérieur du modèle lié au formulaire qui ne sont pas des propriétés de type collection ou complexe.
...
* Utilisez le <xref:Microsoft.AspNetCore.Components.Forms.EditContext> du formulaire pour assigner le modèle lorsque le composant est initialisé.
...
* Décrochez le gestionnaire d’événements dans la méthode `Dispose`. Pour plus d'informations, consultez <xref:blazor/components/lifecycle#component-disposal-with-idisposable>.
...
Dans l’exemple précédent, affectez `false` à la valeur `formInvalid` si :
...
L’un des effets secondaires de l’approche précédente est qu’un composant <xref:Microsoft.AspNetCore.Components.Forms.ValidationSummary> est rempli avec des champs non valides une fois que l’utilisateur interagit avec un champ quelconque. Ce scénario peut être traité de l’une des manières suivantes :

* N’utilisez pas un composant <xref:Microsoft.AspNetCore.Components.Forms.ValidationSummary> sur le formulaire.
* Rendre le composant <xref:Microsoft.AspNetCore.Components.Forms.ValidationSummary> visible lorsque le bouton Envoyer est sélectionné (par exemple, dans une méthode `HandleValidSubmit`).
